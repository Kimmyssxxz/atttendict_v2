      ...
      <main class="flex-1 px-4 sm:px-8 py-6">
        <section class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 overflow-hidden">
          <div class="mb-6">
            ...
            <p class="text-[11px] text-blue-600 mt-2 font-medium flex items-center gap-1.5 bg-blue-50/50 w-fit px-3 py-1.5 rounded-lg border border-blue-100">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
              </svg>
              Note: Click the <span class="font-bold">Status Badge</span> (Active/Inactive) in the table to toggle account access.
            </p>
          </div>
...
export default {
  ...
  methods: {
    ...
    async openRFIDModal(staff) {
      ...
      try {
        await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/api/rfid/register-mode`, {
          method: 'POST',
    ...
    async handleSaveRFID() {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/api/rfid/register-manual`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
