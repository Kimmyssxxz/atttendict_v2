  ...
  methods: {
    ...
    async handleSaveTagging() {
      ...
      try {
        ...
        try {
          await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/admin/attendance/update-tagging`, {
            method: 'POST',
            headers: {
          ...
          if (amTag) {
            await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3001'}/admin/attendance/retag-session`, {
              ...baseOptions,
              body: JSON.stringify({
                internId,
                session: 'AM',
